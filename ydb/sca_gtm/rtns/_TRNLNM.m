%TRNLNM	;;Library; Translate logical names for device references
	...
	;-----Revision History-------------------------------------------------
	;
	; 2012-10-02, Frans S.C. Witte, PRFMUTILS-9
	;	* Modified CDIR() to use $ZDIR instead of &extcall.pwd()
	;
...
BLDPATH(PATH,FILE)	;Private; Get/create the absolute path 
	...
	;
	;----------------------------------------------------------------------
CDIR(FILE)	;Public; Get the absolute value of the current working directory
	;----------------------------------------------------------------------
	; This function will return the absolute value of the directory name 
	; where the GT.M process was started.
	;
	; Note that the GT.M documentation refers to this value as "the default
	; directory", which is inherited from the parent unless assigned
	; explicitly by the job-parameter DEFAULT.
	;
	; @param noreq noret String FILE
	;		File name, if included will be appended to the
	;		directory path.
	;
	;----------------------------------------------------------------------
	NEW PWD
	SET PWD=$EXTRACT($ZDIR,1,$LENGTH($ZDIR)-1)	; trim trailing slash from
	IF $DATA(FILE) SET PWD=$$BLDPATH(PWD,FILE)	; get/create the absolute path
 	QUIT PWD
	;
	;----------------------------------------------------------------------
