%READ	;M Utility;Generic Prompting Routine for M Utilities
	...
	;
	;---- Revision History ------------------------------------------------
	; 2010-10-13, Frans S.C. Witte on behalf of Richard Silvagni, CR51279
	;	Updated GETSTRE to save and set ZTRAP to HALT. If a
	;	reset of the device failed, the process will halt.
	;
...
GETSTR1	R *X:3600
	...
	;
GETSTRE	; fall-through, GT.M runtime exception, IO exception, or read time-out
	;
	; Try to reset the principal device before returning to the caller.
	; If that results in an exception HALT the process.
	;
	; Note that a less radical treatment of an exception in this section
	; would be to set $ZTRAP="QUIT", which basically ignores the failure
	; and returns to the caller. However, this assumes that there will be
	; no repeated calls, as these are likely to fail as well.
	;
	N $ZT
	S $ZT="HALT"
	U $P:(ECHO:EXC="")
