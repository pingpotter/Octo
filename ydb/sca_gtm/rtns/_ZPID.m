%ZPID(LIST)	;Public;Return list of all processes on system
	...
	;
	;---- Revision History ------------------------------------------------
	; 02/28/08 - GRAYJ - CR32291
	;            Retrofit of the following onsite modifications:
	;            04/19/05 - Rick Silvagni 
	;                 Modified section VALID by replacing calls to build
	;                 an array of process id's to instead use a single
	;                 UNIX ps command, or if GT.M 4.4-004 and greater use
...
VALID(PID,vzhex)	;Public;Is PID a valid process on the system?
	...
	Q:+$TR($P($zvn,"GT.M V",2),".-",".")'<4.4004 $ZGETJPI(PID,"ISPROCALIVE")   ;JG CR32291
	Q '$$SYS^%ZFUNC("ps -p "_PID_" >/dev/null 2>&1")
	;
	;----------------------------------------------------------------------
