...
void prunning(int count,char *process_name,SLONG *return_code)
{
...
void pgtm(int count,SLONG pid,SLONG *return_code)
{
	RETURNSTATUS 	rc = SUCCESS;
	FILE 		*fd = (FILE *)NULL;
	char 		cmd[MAX_CMD_LEN];
...
void listpids(int count, SLONG *pid, SLONG *return_code)
{
	RETURNSTATUS	rc = SUCCESS;
 ...
 ****/
void gettime(int count,char *time_str)
{
...
void extsleep1(int count,SLONG timeout)
{
	/*
	*	Sleep for timeout
	*/
	/*
	*	DAG Sep 24, 98. 
	*	Sleep changed to sca_sleep to guarantee it will sleep for
	*	timeout seconds.
	*/
	sca_sleep(timeout);
...
void extsleep(int count,SLONG timeout,int(*sleep_noint)())
{
         /* signal(SIGUSR2,quit); */
        /*
        *       Sleep for timeout
...
void sendsig(int count,SLONG pid,SLONG signalno)
{
	/*
...
void sendsvsig(int count,SLONG pid)
{
        /*
	...
	SLONG *return_code)
{
	...
	*/
	if((rc = fscanf(fd, "%s", result->str)) < 1)
	{
#ifdef DEBUG
	do {
 ...
 *====================================================================*/
int PostEventSanchez(char *evname, int evpriority, char *evformat)
{	
	return 0;
}
...
int GtmEventLog(int count, char *category, char *code, char *msg)
{	
	SLONG 		rc;
	char		desc_str[2048];
	STR_DESCRIPTOR	desc;
