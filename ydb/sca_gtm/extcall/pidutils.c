...
void getgroup(int count,SLONG *pid,SLONG *return_code)
{
...
void getusername(int count,char *response,SLONG *return_code)
{
	struct passwd   *pwd_entry = (struct passwd *)NULL;
...
void ttgetusername(int count,STR_DESCRIPTOR *response,SLONG *return_code)
{
...
void tgetusername(int count,STR_DESCRIPTOR *response,SLONG *return_code)
{
	register int 	i = 0;
	SLONG			pid = 0;
...
void geterminal(int count,SLONG *pid,STR_DESCRIPTOR *response,SLONG *return_code)
{
	register int i = 0;
...
void getmember(int count,SLONG *pid,STR_DESCRIPTOR *response,SLONG *return_code)
{
	RETURNSTATUS	rc = SUCCESS;
	register int 	i = 0;
	struct passwd	*pwd_entry = (struct passwd *)NULL;
...
void getprocid(int count,SLONG *pid,STR_DESCRIPTOR *response,SLONG *return_code)
{
...
void getimage(int count,SLONG *pid,STR_DESCRIPTOR *response,SLONG *return_code)
{
	register int i = 0;
...
void getprcnam(int count,SLONG *pid,STR_DESCRIPTOR *response,SLONG *return_code)
{
	register int 	i = 0;
...
void getlogintime(int count,SLONG *pid,STR_DESCRIPTOR *response,SLONG *return_code)
{
	register int i = 0;
...
void getcputime(int count,SLONG *pid,STR_DESCRIPTOR *response,SLONG *return_code)
{
	register int i = 0;
...
void getparentpid(int count,SLONG *pid,SLONG *ppid, SLONG *return_code)
{
	FILE 	*fd = (FILE *)NULL;
	...
	*return_code = SUCCESS;
	...
	/*
	* Call getppid and return if process id wasn't passed in, or
...
void getjobtype(int count,SLONG *pid,SLONG *return_code)
{
	register int	i = 0;
	...
	for(i=0;i<PST_MAX_PROCS;i++)
	{
		if(*pid == pid_list[i].pid)
		{
			if((*return_code = job_type(*pid, pid_list[i].tty, return_code)) == FAILURE)
				return;
			break;
		}
	}
...
SLONG job_type(SLONG pid,char *tty,SLONG *return_code)
{
	RETURNSTATUS 	rc = SUCCESS;
	FILE 		*fd = (FILE *)NULL;
	char 		cmd[128];
...
void create_pid_list(SLONG *return_code)
{
	RETURNSTATUS 	rc = SUCCESS;
	FILE 		*fd = (FILE *)NULL;
...
void permissions(int count,char *permission,SLONG *return_code)
{
	RETURNSTATUS	rc = SUCCESS;
	struct group	*grp_entry = (struct group *)NULL;
	char		grp_name[24];
