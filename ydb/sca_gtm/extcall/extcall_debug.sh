...
#	Initial revision
#	Script for creating Debug library for extcall package.
...
case $os in
	"Linux")
		...
		if [ "$os_bit" = "x86_64" ]
		then
		rm -f *.o *.sl *.a extcall_linux_64.tar extcall_linux_64.tar.gz
		make -f extcall.mk "CC=/usr/bin/gcc" "CFLAGS = -c -g -fpic -DDEBUG" "LD=/usr/bin/gcc" "LFLAGS= -lm -lc" "SHARED_LIBRARY=extcall.sl" "LIBNAME= -shared -o"
 		rm -f extcallversion
		...
		rm -f *.o *.sl *.a
		make -f extcall.mk "CC=/usr/bin/gcc" "CFLAGS = -c -g -DDEBUG" "LD=gcc" "LFLAGS=-lm" "SHARED_LIBRARY=extcall.sl" "LIBNAME= -shared -o"
 		rm -f extcallversion
 		make -f version.mk "SHARED_LIBRARY=extcall.sl" 
	...
	"AIX")
		...
		then	
		rm -f *.o *.sl *.a  
		make -f extcall.mk "CC=/usr/vac/bin/cc" "CFLAGS = -c -q64 -DDEBUG" "LD=/usr/bin/ld" "LFLAGS=-b64 -bE:extcall.exp -H512 -T512 -bM:SRE -lc -bh:4 -lm -eclxfr" "SHARED_LIBRARY=extcall.a" "LIBNAME= -o" 
        rm -f extcallversion
		...
		rm -f *.o *.sl *.a  
		rm -f extcallversion
 		make -f extcall.mk "CC=/usr/vac/bin/cc" "CFLAGS = -c -g -DDEBUG" "LD=/usr/bin/ld" "LFLAGS=-bE:./extcall.exp -H512 -T512 -bM:SRE -lc -bh:4 -lm -eclxfr" "SHARED_LIBRARY=extcall.a" "LIBNAME= -o" 
